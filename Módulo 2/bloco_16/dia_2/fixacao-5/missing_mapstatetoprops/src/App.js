...
class App extends React.Component {
  ...
  first() {
    const { spiedIngredients } = this.props;
    return (
    ...
    return (
      <div className="flip-card-inner">
        ...
        <ul className="flip-card-back list-group">
          ...
          { spiedOrder.map((element, index) => <li key={ index }>{ element }</li>) }
        </ul>
      </div>
    );
  }
...
const mapStateToProps = (state) => ({
  spiedIngredients: state.spiedIngredients.ingredients,
  spiedOrder: state.spiedOrder.order,
});

App.propTypes = {
  spiedIngredients: PropTypes.arrayOf(PropTypes.string).isRequired,
  spiedOrder: PropTypes.arrayOf(PropTypes.string).isRequired,
};
