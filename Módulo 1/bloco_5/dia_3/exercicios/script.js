  ...
  getFridayButton.addEventListener('click', function() {
    for (let index = 0; index < getFridays.length; index += 1) {
      if (getFridays[index].innerText === 'SEXTOU!') {
        getFridays[index].innerText = index+1;
      } else {
...
const diaMes = document.querySelectorAll('.day');

diaMes.addEventListener('mouseover', zoonIn);

function zoonIn(event) {  
  //for (let index = 0; index < diaMes.length; index += 1) {
    event.target = 'zoom';
    //}
  }

  diaMes.addEventListener('mouseout', zoomOut);